...
export default function PainelEquipe({ selecionados, onRemover, onLimpar }) {
  if (selecionados.length === 0) return null

  // Para cada competência, pega o MÁXIMO entre os selecionados
  const cobertura = ITENS.map(({ key, label }) => {
    const max = Math.max(...selecionados.map(f => parseFloat(f[key] ?? 0)))
    return { key, label, max }
  })

  const gaps      = cobertura.filter(c => c.max < 5)
  const atencao   = cobertura.filter(c => c.max >= 5 && c.max < 7)
  const equilibrio = (cobertura.reduce((s, c) => s + c.max, 0) / cobertura.length).toFixed(1)
