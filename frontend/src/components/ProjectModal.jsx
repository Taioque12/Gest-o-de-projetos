...
import AbaSST from './project-modal/AbaSST'
import KanbanBoard from './KanbanBoard'
import { gerarRelatorioPDF } from '../utils/pdfGenerator'

const TABS = ['Visão Geral', 'Comparativo', 'Histograma', 'Programação', 'Kanban', 'Diário', 'Suprimentos', 'Qualidade', 'SST (Segurança)', 'Histórico', 'Anexos', 'Análise IA']
    ...
    <div className="overlay open" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" style={{ maxWidth: 1100, width: '95%', padding: 0, display: 'flex', height: '85vh', maxHeight: 900, overflow: 'hidden' }} role="dialog" aria-modal="true" aria-labelledby="project-modal-titulo" tabIndex={-1} ref={el => el?.focus()}>
        ...
        <div className="modal-body" id="pdf-export-area" style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
          ...
          {aba === 'Suprimentos' && <AbaSuprimentos projetoId={p.id} podeEditar={podeEditar} />}
          {aba === 'Qualidade' && <AbaQualidade projetoId={p.id} podeEditar={podeEditar} />}
          {aba === 'SST (Segurança)' && <AbaSST projetoId={p.id} podeEditar={podeEditar} />}

          {aba === 'Histórico' && <AbaHistorico p={p} hist={hist} />}
