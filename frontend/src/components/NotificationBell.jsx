    ...
    <div style={{ position: 'relative', marginRight: 16 }}>
      ...
      >
        ...
        {notificacoes.length > 0 && (
          <span aria-live="polite" style={{ position: 'absolute', top: -5, right: -5, background: 'var(--vermelho)', color: 'white', fontSize: 10, fontWeight: 'bold', padding: '2px 6px', borderRadius: 999 }}>
            {notificacoes.length}
          </span>
        )}
      </button>
            ...
            {notificacoes.length === 0 ? (
              ...
              notificacoes.map(n => (
                <div key={n.id} style={{ padding: '12px 14px', borderBottom: '1px solid var(--line)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <button aria-label="Marcar como lida" onClick={() => { marcarLida(n.id); if (notificacoes.length === 1) setOpen(false); }} style={{ background: 'none', border: 'none', color: 'var(--brand)', cursor: 'pointer', padding: 4 }}>
